...
setwd("results_10stop")

simdata <- simdata[1:10]
missing_MAR_list <- missing_MAR_list[1:2]
...
All_param_set_maxit5 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
        params <- xgb_param_calc(mat,response = "all", select_features=NULL, iter = 50)
      })
...
save(All_param_set_time_maxit5, file = "results_iter50/xgb_ParamAll_maxit5_P_param_time_50iter.RData")
save(All_param_set_parameter_maxit5, file = "results_iter50/xgb_ParamAll_maxit5_P_param_50iter.RData")
...
xgb_ParamAll_maxit5_PO_imp_eval_linear <- xgb_ParamAll_maxit5_PO_imp_res %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_res, file = "results_iter50/xgb_ParamAll_maxit5_PO_imp_res_iter50.RData")
save(xgb_ParamAll_maxit5_PO_imp_time, file = "results_iter50/xgb_ParamAll_maxit5_PO_imp_time_iter50.RData")
save(xgb_ParamAll_maxit5_PO_imp_eval, file = "results_iter50/xgb_ParamAll_maxit5_PO_imp_eval_iter50.RData")
save(xgb_ParamAll_maxit5_PO_imp_eval_linear, file = "results_iter50/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter50.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res, xgb_ParamAll_maxit5_PO_imp_time,xgb_ParamAll_maxit5_PO_imp_eval, 
   xgb_ParamAll_maxit5_PO_imp_eval_linear, xgb_ParamAll_maxit5_PO_imp_res_tmp,All_param_set_time_maxit5, 
   All_param_set_parameter_maxit5)

########################## iter = 100 ############################


all_param_set_maxit5_iter100 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
...
all_param_set_parameter_maxit5_iter100 <- map(all_param_set_maxit5_iter100, ~ map(., "params"))  # imputation results
all_param_set_time_maxit5_iter100 <- map(all_param_set_maxit5_iter100, ~ map(., "time_taken"))           # Time taken

save(all_param_set_time_maxit5_iter100, file = "results_iter100/xgb_ParamAll_maxit5_P_param_time_100iter.RData")
save(all_param_set_parameter_maxit5_iter100, file = "results_iter100/xgb_ParamAll_maxit5_P_param_100iter.RData")


xgb_ParamAll_maxit5_PO_imp_res_tmp_iter100 <- future_map2(missing_MAR_list, all_param_set_parameter_maxit5_iter100, 
                                                          .f = function(data_inner, params_inner) {
                                                            future_map2(data_inner, params_inner, 
                                                                        .f = function(data_single, params_single) {
                                                                          result <- system.time({
...
xgb_ParamAll_maxit5_PO_imp_res_iter100 <- map(xgb_ParamAll_maxit5_PO_imp_res_tmp_iter100, ~ map(., "mice_result"))  # imputation results
xgb_ParamAll_maxit5_PO_imp_time_iter100 <- map(xgb_ParamAll_maxit5_PO_imp_res_tmp_iter100, ~ map(., "time_taken"))  # Time taken


xgb_ParamAll_maxit5_PO_imp_eval_iter100 <- xgb_ParamAll_maxit5_PO_imp_res_iter100 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
xgb_ParamAll_maxit5_PO_imp_eval_linear_iter100 <- xgb_ParamAll_maxit5_PO_imp_res_iter100 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_res_iter100, file = "results_iter100/xgb_ParamAll_maxit5_PO_imp_res_iter100.RData")
save(xgb_ParamAll_maxit5_PO_imp_time_iter100, file = "results_iter100/xgb_ParamAll_maxit5_PO_imp_time_iter100.RData")
save(xgb_ParamAll_maxit5_PO_imp_eval_iter100, file = "results_iter100/xgb_ParamAll_maxit5_PO_imp_eval_iter100.RData")
save(xgb_ParamAll_maxit5_PO_imp_eval_linear_iter100, file = "results_iter100/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter100.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res_iter100, xgb_ParamAll_maxit5_PO_imp_time_iter100,xgb_ParamAll_maxit5_PO_imp_eval_iter100, 
   xgb_ParamAll_maxit5_PO_imp_eval_linear_iter100, xgb_ParamAll_maxit5_PO_imp_res_tmp_iter100,all_param_set_time_maxit5_iter100, 
   all_param_set_parameter_maxit5_iter100 )
...
all_param_set_maxit5_iter150 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
        params <- xgb_param_calc(mat,response = "all", select_features=NULL, iter = 150)
      })
      list(params = params, time_taken = result)
    }, .options = furrr_options(seed = TRUE))
  })
...
xgb_ParamAll_maxit5_PO_imp_eval_iter150_linear <- xgb_ParamAll_maxit5_PO_imp_res_iter150 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_eval_iter150_linear, file = "results_iter150/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter150.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res_iter150, xgb_ParamAll_maxit5_PO_imp_time_iter150,xgb_ParamAll_maxit5_PO_imp_eval_iter150, 
   xgb_ParamAll_maxit5_PO_imp_eval_iter150_linear, xgb_ParamAll_maxit5_PO_imp_res_tmp_iter150,all_param_set_time_maxit5_iter150, 
   all_param_set_parameter_maxit5_iter150 )
...
all_param_set_maxit5_iter200 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
        params <- xgb_param_calc(mat,response = "all", select_features=NULL, iter = 200)
      })
      list(params = params, time_taken = result)
    }, .options = furrr_options(seed = TRUE))
  })
...
xgb_ParamAll_maxit5_PO_imp_eval_iter200_linear <- xgb_ParamAll_maxit5_PO_imp_res_iter200 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_eval_iter200_linear, file = "results_iter200/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter200.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res_iter200, xgb_ParamAll_maxit5_PO_imp_time_iter200,xgb_ParamAll_maxit5_PO_imp_eval_iter200, 
   xgb_ParamAll_maxit5_PO_imp_eval_iter200_linear, xgb_ParamAll_maxit5_PO_imp_res_tmp_iter200,all_param_set_time_maxit5_iter200, 
   all_param_set_parameter_maxit5_iter200 )
...
all_param_set_maxit5_iter250 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
        params <- xgb_param_calc(mat,response = "all", select_features=NULL, iter = 250)
      })
      list(params = params, time_taken = result)
    }, .options = furrr_options(seed = TRUE))
  })
...
xgb_ParamAll_maxit5_PO_imp_eval_iter250_linear <- xgb_ParamAll_maxit5_PO_imp_res_iter250 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_eval_iter250_linear, file = "results_iter250/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter250.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res_iter250, xgb_ParamAll_maxit5_PO_imp_time_iter250,xgb_ParamAll_maxit5_PO_imp_eval_iter250, 
   xgb_ParamAll_maxit5_PO_imp_eval_iter250_linear, xgb_ParamAll_maxit5_PO_imp_res_tmp_iter250,all_param_set_time_maxit5_iter250, 
   all_param_set_parameter_maxit5_iter250 )
...
all_param_set_maxit5_iter300 <- missing_MAR_list %>%
  map(function(mat_list) { 
    furrr::future_map(mat_list, function(mat) {
      result <- system.time({
        params <- xgb_param_calc(mat,response = "all", select_features=NULL, iter = 300)
      })
      list(params = params, time_taken = result)
    }, .options = furrr_options(seed = TRUE))
  })
...
xgb_ParamAll_maxit5_PO_imp_eval_iter300_linear <- xgb_ParamAll_maxit5_PO_imp_res_iter300 %>% 
  map(function(mat_list) {
    furrr::future_map(mat_list, function(mat) {
      complete(mat, "all") %>% # create a list of completed data sets
        map(~.x %$% # for every completed data set....
...
save(xgb_ParamAll_maxit5_PO_imp_eval_iter300_linear, file = "results_iter300/xgb_ParamAll_maxit5_PO_imp_eval_linear_iter300.RData")

rm(xgb_ParamAll_maxit5_PO_imp_res_iter300, xgb_ParamAll_maxit5_PO_imp_time_iter300,xgb_ParamAll_maxit5_PO_imp_eval_iter300, 
   xgb_ParamAll_maxit5_PO_imp_eval_iter300_linear, xgb_ParamAll_maxit5_PO_imp_res_tmp_iter300,all_param_set_time_maxit5_iter300, 
   all_param_set_parameter_maxit5_iter300 )
