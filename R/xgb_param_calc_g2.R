...
xgb_param_calc_g2<-function(data,response = NULL, select_features=NULL, num_cores = 1, iter=50){
  ...
  if(length(response_var)>1){
    ...
    params <- lapply(response_var,function(x) par_opt_g2(cc.data, setdiff(Names, x), x, iter = iter)) # call to bayesian optimization using assigned features and response variable
    
  } else {
    
    message("Performing bayesian optimization using ", response_var, " as a response variable and ", training_features, " as features")
    params = par_opt_g2(cc.data, training_features, response_var,iter = iter)
  }
...
par_opt_g2 <- function(cc.data, training_features, response_var, nfold = 5, iter = iter, early_stopping_rounds = 10, nround = 100){
