#' Creates a \code{xgb_param_calc_g3} argument
#'
#' This helper function creates a valid \code{xgb_param_calc} vector. The
#' \code{xgb_param_calc_g3} vector is an argument to the \code{mice} function that
...
xgb_param_calc_g3<-function(data,response = NULL, select_features=NULL, num_cores = 1, iter=50){
  ...
  if(length(response_var)>1){
    ...
    params <- lapply(response_var,function(x) par_opt_g3(cc.data, setdiff(Names, x), x, iter = iter)) # call to bayesian optimization using assigned features and response variable
    
  } else {
    
    message("Performing bayesian optimization using ", response_var, " as a response variable and ", training_features, " as features")
    params = par_opt_g3(cc.data, training_features, response_var,iter = iter)
  }
...
par_opt_g3 <- function(cc.data, training_features, response_var, nfold = 5, iter = iter, early_stopping_rounds = 10, nround = 100){
  ...
  obj.fun  <- smoof::makeSingleObjectiveFunction(
    ...
    par.set = makeParamSet(  #setting bounds of parameter which are being optimized
      ...
      makeIntegerParam("n_estimators", lower = 50L, upper = 500L), # Adjust the range as needed
      #makeNumericParam("alpha",lower = 1e-9, upper = 100), #2 change from 100
      #makeNumericParam("lambda",lower = 1e-9, upper = 100), #2 change from 100
      makeNumericParam("gamma",lower = 1e-9, upper = 1), # . 1: chqnged from 10 - 1
      makeIntegerParam("max_depth",lower= 2L,upper = 20L), #. 1: check from github
      makeIntegerParam("min_child_weight", lower= 1L,upper = 20L), #1 : check from github
  ...
  des = generateDesign(n= (4* getNumberOfParameters(obj.fun)),
  ...
  if(continue==TRUE){
    ...
    best_parameters <- list("eta" = 0.3, "n_estimators" = 100)
    plot_fig <- NA
    
  }
  list("parameter" = best_parameters, "fig" = plot_fig, "responseVariable" = response_var)
